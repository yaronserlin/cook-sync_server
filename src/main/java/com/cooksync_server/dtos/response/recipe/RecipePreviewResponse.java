...
/**
 * Data Transfer Object for a recipe preview/card response. Used in lists (home
 * screen, search results) to save bandwidth by omitting heavy data
 * (ingredients, instructions). Uses Java records for an immutable data carrier.
 */
public record RecipePreviewResponse(
    ...
    /**
     * Maps a persistent Recipe entity to a lightweight RecipePreviewResponse
     * DTO.
     */
    public static RecipePreviewResponse fromEntity(Recipe recipe) {
        return new RecipePreviewResponse(
                recipe.getId(),
                recipe.getCreatedBy() != null ? recipe.getCreatedBy().getFullName() : null,
                recipe.getTitle(),
